...
export default function Tariffs() {
  ...
  const loadTariffs = async () => {
    try {
      ...
      const mapped = data.map((f) => {
        const validFrom = f.startDate
          ? moment(f.startDate.toDate()).format("YYYY-MM-DD")
          : null;

        const validTo = f.endDate
          ? moment(f.endDate.toDate()).format("YYYY-MM-DD")
          : null;
  ...
  const onCreate = () => {
    const payload = {
      ...
      stratum: Number(form.stratum),
      price: Number(form.priceCents),
      startDate: form.validFrom ? new Date(form.validFrom) : null,
      endDate: form.validTo ? new Date(form.validTo) : null,
      status: "Active",
    };
    feeService.feeCreate(payload);
    setRows((rs) => [{ ...form, id: crypto.randomUUID() }, ...rs]);
    setOpen(null);
    setForm({ stratum: 1, active: true });
  };
  const onUpdate = () => {
        ...
        <div className={ui.modalBackdrop}>
          <div className={ui.modalCard}>
            <div className={ui.modalHeader}>
              {open === "create" ? "Nueva tarifa" : "Editar tarifa"}
            </div>
            <div className={ui.formGrid}>
              <select
                className={ui.select}
                value={form.stratum ?? 1}
                onChange={(e) =>
                  setForm((f) => ({ ...f, stratum: Number(e.target.value) }))
                }
              >
                {[1, 2, 3, 4, 5, 6].map((s) => (
                  <option key={s} value={s}>
            ...
            <div className={ui.footer}>
              ...
              <button
                className={`${ui.button} ${ui.primary}`}
                onClick={open === "create" ? onCreate : onUpdate}
              >
                Guardar
              </button>
            </div>
